for i = 0:30
    ...
    time = 150*3600/t_factor; % Time of simulation depending on t_factor.
    ...
    SSRI_dose_factor = 2.06;                       % mg/kg of body weight. 
    ...
    SSRI_repeat_time = 24*3600/t_factor; %Time for repeat of dose. 
    ...
    FMH_repeat_time = 10000*3600/t_factor; %Time for repeat of dose. 
    ...
    %% Model Solving. 
     [T,Y] = ode45(@msc, time_array, [95.9766 0.0994	0.9006	20.1618	1.6094	0.0373	63.0383 280.0048	0.0603	1.6824	113.4099 0.8660	1.0112	0.9791	0.0027	0.7114	1.3245	0.9874	0.2666 1.0203 	0.2297	0	0	0	0	0	3.1074	136.3639 241.9217	1.4378	2.0126	99.7316	249.3265	311.6581	0.7114	1.3245	0.9874	0.8660	1.0112	0.9791	354.6656	177.3328	350	150	3	140	0.7205	1.3539	1.0051 0 0 0 0 1],[], v2, ssri_molecular_weight, SSRI_start_time, SSRI_repeat_time, SSRI_dose*SSRI_bioavailability, fmh_molecular_weight, FMH_start_time, FMH_repeat_time, FMH_dose*FMH_bioavailability,  mc_switch, mc_start_time, btrp0, eht_basal, gstar_5ht_basal, gstar_ha_basal, bht0, vht_basal, vha_basal);
    
    %% Extracting and calculating parameters. 
    %Getting the ssri array in concentration.
    ssri_array = (Y(:,25)/v2)*1000/ssri_molecular_weight; % uM. 
    ssri_array_blood = (Y(:,24)/v1)*1000/ssri_molecular_weight; % uM. 
    
    %% Plotting of results. 
    figure;
    plot(T, 1000.*Y(:,9),'LineWidth',3);
    leg1=legend('eht');
    ...
    % figure;
    % plot(T, Y(:, 54), 'green','LineWidth',3); 
    % leg1 = legend('HTDC ratio');
    % set(leg1,'FontSize',14);
    % xlabel('Time');
    % ylabel('HTDC ratio'); 
